...
void ImGui::TextBackground(std::string text, ImVec2 pos) {
	ImGui::GetBackgroundDrawList()->AddText(pos, GetColorU32(ImGuiCol_Text), text.c_str());
}
...
void ImGui::InputTextPositioned(const char* label, char* buf, size_t buf_size, ImVec2 position, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data) {
...
ImVec2 ImGui::operator-(const ImVec2& l, const ImVec2& r) { return{ l.x - r.x, l.y - r.y }; }
...
template<ImGui::ElementsEnum ElemT>
bool ImGui::UIRendererUtils::isAllElementsOr(ImGui::ElementsEnum in) {
...
void ImGui::ElementStubImpl::RenderFinishDrawingImpl() { ElementOffset = 0; }

void ImGui::ElementStubImpl::RenderBeginImpl() {
	ImGuiIO& io = ImGui::GetIO();
	ImGui::SetNextWindowPos({ 0,0 });
